...
var Wizard = (function () {
    var n = document,
        ...
        f = n.createElement("b"),
        i = [{ k: "", n: chrome.i18n.getMessage("optionsProfilesMenuUnfiled") }],
        ...
        s = chrome.runtime.connect({ name: "wizard" });
    return {
        addCat: function () {
            ...
            if (n)
                if (n.length > 60) alert("[" + sExtName + "] " + chrome.i18n.getMessage("statusProfileCharLimit")), (t.selectedIndex = u);
                    ...
                    try {
                        chrome.runtime.sendMessage({ type: "saveCat", data: { cat: { n: n } } }, function () {});
                    } catch (r) {
                        alert(sBgUpdErr);
                    }
                }
        ...
        close: function () {
            ...
            try {
                chrome.runtime.sendMessage({ type: "clearHighlight" });
            } catch (i) {
                alert(sBgUpdErr);
                return;
            }
        ...
        create: function () {
            try {
                chrome.runtime.sendMessage({ type: "gatherRules", data: t.value });
            } catch (n) {
                alert(sBgUpdErr);
            }
        },
        popCats: function () {
            i.length = 1;
            i = i.concat(oLS.cats);
            i.push({ k: "new", n: chrome.i18n.getMessage("optionsProfilesMenuNew") });
            t.length = 0;
            for (var r = 0, f; r < i.length; ++r)
                (f = n.createElement("option")), (f.value = i[r].k), (f.text = i[r].n), i[r].n.indexOf(sBackup) === 0 ? (f.hidden = !0) : r === u ? (f.selected = !0) : r === i.length - 1 && (f.className = "menu-action"), t.add(f);
        },
        selCat: function () {
        ...
        handleMessage: function (t) {
            ...
            switch (t.type) {
                case "gotRules":
                    ...
                    r.length
                        ? ((e = /([$^.?+*\\|(){}\[\]])/g),
                          (f = n.querySelector("#autofill-wizard-step3>button")),
                          (f.disabled = !0),
                          (f.style.color = "#999"),
                          chrome.runtime.sendMessage({ type: "saveRules", data: { rules: r } }),
                          Wizard.setStatus(0, chrome.i18n.getMessage("statusDone")),
                          oLS.other.voice && ((i = new Audio(chrome.runtime.getURL("sounds/" + u[oLS.voice < 6 ? oLS.voice : R(0, 5)] + ".ogg"))), i.play(), delete i),
                          setTimeout(Wizard.close, 2e3))
                        : (Wizard.setStatus(1, t.field ? chrome.i18n.getMessage("statusEnterValue") : chrome.i18n.getMessage("statusNoChange")),
                          oLS.other.voice && ((i = new Audio(chrome.runtime.getURL("sounds/" + u[oLS.voice < 6 ? oLS.voice : R(0, 5)] + "-err.ogg"))), i.play(), delete i));
            }
        },
    };
})();
