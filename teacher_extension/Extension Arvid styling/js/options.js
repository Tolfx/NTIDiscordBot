...
function activateTab(n) {
    var t = "fields",
        ...
        u = "textclips",
        f = "variables";
    if ((n && n.target && (n = this.id.slice(4)), (location.hash = n), (D.body.id = n), n === t || n === i || n === r || n === u || n === f))
        switch (n) {
            case i:
                setTimeout(function () {
                    oTAA.focus();
    ...
    chrome.storage.local.get(function (i) {
        ...
        if (
            (t &&
                ...
                (W = D.body.offsetWidth),
                (H = D.documentElement.clientHeight),
                (oNF = D.getElementById("nav-fields")),
                (oNA = D.getElementById("nav-advanced")),
                (oNE = D.getElementById("nav-exceptions")),
                (oNT = D.getElementById("nav-textclips")),
                (oNV = D.getElementById("nav-variables")),
                (oNIE = D.getElementById("nav-importexport")),
                (oNO = D.getElementById("nav-other")),
                (oNS = D.getElementById("nav-support")),
                (oND = D.getElementById("nav-datasize")),
                (oNW = D.getElementById("nav-whatsnew")),
                (oHN = D.getElementById("help-name")),
                (oHV = D.getElementById("help-value")),
                (oHS = D.getElementById("help-site")),
                (oHM = D.getElementById("help-mode")),
                (oBA = D.getElementById("button-add")),
                (oBS = D.getElementById("button-save")),
                (oBR = D.getElementById("button-reset")),
                (oBH = D.getElementById("button-help")),
                (oBI = D.getElementById("button-import")),
                (oBE = D.getElementById("button-export")),
                (oBIU = D.getElementById("button-importurl")),
                (oT = D.getElementById("content-fields")),
                (oTB = D.getElementById("content-fields-body")),
                (oC = D.getElementById("content-cats")),
                (oCS = D.getElementById("content-site")),
                (oCH = D.getElementById("content-hotkey")),
                (oTAA = D.getElementById("content-advanced")),
                (oTAE = D.getElementById("content-exceptions")),
                (oTAT = D.getElementById("content-textclips")),
                (oTAV = D.getElementById("content-variables")),
                (oIE = D.getElementById("content-ie")),
                (oIR = D.getElementById("radio-replace")),
                (oIA = D.getElementById("radio-append")),
                (oIU = D.getElementById("content-importurl")),
                (oOO = {
                ...
                (oBU = D.getElementById("content-backup-chars")),
                (oD = D.getElementById("content-delay-sec")),
                (oDO = D.getElementById("content-delay-out")),
                (oV = D.getElementById("content-voice-sel")),
                (oS = D.getElementById("status")),
                (oST = oS.getElementsByTagName("span")[0]),
                (oL = D.getElementById("light")),
                (oB = D.getElementById("box")),
                (oTT = D.getElementById("tt")),
                (oErr.isOn = function () {
                    return oST.textContent.length > 0 && oST.style.background === sErrBg;
                }),
                (oErr.refresh = function () {
                    for (var n = 1, t; n < oT.rows.length; ++n)
                        (t = oT.rows[n].id.slice(2)), (oErr.fields[n] = D.getElementById("n_" + t).style.background || D.getElementById("v_" + t).style.background || D.getElementById("s_" + t).style.background ? 1 : "");
                    err("fields") || showStatus();
                }),
                (n = location.hash.slice(1)),
                D.getElementById("tab-" + n) && activateTab(n),
                sVer > (i.ver || "1.0.0") && (showWhatsNew(), storageSet({ ver: sVer })),
                resizeTable(),
                (D.body.style.visibility = "visible")),
            oErr.isOn() && showStatus(),
            (n === "fields" || t) && ((oErr.fields = []), (aC = i.cats), (nC = i.catcount), (nCI = nCI2 = i.catnow), (oR = JSON.parse(i.rules)), (nR = i.rulecount), popCats(), buildRules(oC.value)),
            (n === "cats" || t) &&
                ((r = nCI < 3),
                (oCS.disabled = r),
                (oCH.disabled = r),
                r
                    ? (oCS.value = oCH.value = "")
                    : ((oCS.value = aC[nCI - 5].s || ""), (oCH.value = aC[nCI - 5].h || ""), (oCH.hotkey = oCH.value), (isBackupCat = oC[oC.selectedIndex].text.indexOf(sBackup) === 0) && (oCS.disabled = oCH.disabled = !0))),
            ...
            D.getElementById("yearnow").textContent = new Date().getFullYear();
...
function openHelp() {
    open("https://www.tohodo.com/autofill/help.html#" + D.body.id, "AutofillHelp");
}
function saveOptions(n) {
    ...
    switch (n) {
        case "fields":
            ...
            popCats();
            storageSet({ rulecount: nR, rules: JSON.stringify(oR) });
        ...
        case "cats":
            ...
            storageSet({ catcount: nC, catnow: nCI2, cats: aC });
        ...
        case "advanced":
            ...
            for (disableButtons(n, !0, !0), oTAA.value = cleanText(oTAA.value), aA = oTAA.value ? oTAA.value.split("\n") : [], t = 0; t < aA.length; ++t)
                (aA[t] = aA[t].replace(/ *: */, ": ").replace(/ *= */g, "=").replace(/ {2,}/g, " ")), /^[cr]\d+:/.test(aA[t]) || aA.splice(t, 1);
            oTAA.value = aA.join("\n");
            storageSet({ advanced: aA });
            break;
        case "exceptions":
            if (err("fields")) return;
        ...
        case "variables":
            ...
            for (disableButtons(n, !0, !0), oTAV.value = cleanText(oTAV.value), aV = oTAV.value ? oTAV.value.split("\n") : [], t = 0; t < aV.length; ++t)
                (aV[t] = aV[t].replace(/ *= */, " = ").replace(/ {2,}/g, " ")), /^\w+ = /.test(aV[t]) || aV.splice(t, 1);
            oTAV.value = aV.join("\n");
            storageSet({ variables: aV });
            break;
        default:
            ...
            switch (n) {
                ...
                case "voice":
                    ...
                    storageSet({ voice: oV.selectedIndex });
                    break;
                case "mask":
...
function setAccessKeys(n) {
    ...
    arguments.callee.caller.name === "showBox" &&
        (oNF.setAttribute(t, n ? "l" : ""),
        oNA.setAttribute(t, n ? "n" : ""),
        oNE.setAttribute(t, n ? "c" : ""),
        oNT.setAttribute(t, n ? "t" : ""),
        oNV.setAttribute(t, n ? "v" : ""),
        oNIE.setAttribute(t, n ? "i" : ""),
        oNO.setAttribute(t, n ? "o" : ""),
        oNS.setAttribute(t, n ? "u" : ""),
        oBH.setAttribute(t, n ? "h" : ""));
}
function setRule(n, t) {
    ...
    f.value = e;
    o.value = u;
    oR["r" + r] = { t: D.getElementById("t_" + r).selectedIndex, n: e, v: D.getElementById("v_" + r).value, s: u, o: D.getElementById("o_" + r).selectedIndex, c: t };
}
...
function addRow(n, t, i, r, u, f, e) {
    ...
    p.innerHTML =
        ...
        n +
        '" title="' +
        chrome.i18n.getMessage("removeRuleTT") +
        " (" +
        sK +
        '+X)">&ndash;</button><button id="m_' +
        n +
        '" title="' +
        (s ? chrome.i18n.getMessage("optionsProfileTT") + ": " + s + "\n" : "") +
        chrome.i18n.getMessage("moveRuleTT") +
        " (" +
        sK +
        '+M)">' +
        (s ? "<span>" + s + "</span> " : "") +
...
function buildRules(n) {
    if ((oT.classList.toggle("view-all", nCI === 1), oT.rows.length > 1)) while (oT.rows.length > 1) oT.deleteRow(-1);
        ...
        (n.nodeName === "TEXTAREA" &&
            (t
                ? n.clientHeight < n.scrollHeight
                    ...
                    : (window.event && window.event.key === "Backspace" && n.rows > 1 && ((n.rows = 1), (n.rows = Math.min(Math.round(n.scrollHeight / 16), 10))), n.scrollIntoViewIfNeeded())
                : n.rows > 1 &&
                  !bM &&
                  setTimeout(function () {
...
function moveRow(n, t) {
    ...
    switch (t.key) {
        case "ArrowUp":
            t.altKey
                ? ((i = D.getElementById("r_" + n.id.slice(2))),
                  i.rowIndex > 1 && ((bM = !0), (n.readOnly = !0), i.parentNode.insertBefore(i, i.previousSibling), (bM = !1), err("fields") ? oErr.refresh() : disableButtons("fields", !1, !1), n.focus()))
                : n.parentNode.parentNode.previousSibling.id &&
                  (n.type !== "textarea" || n.selectionEnd - n.selectionStart === n.value.length || n.selectionStart === 0) &&
                  (t.preventDefault(),
                  (r = D.getElementById(n.id.slice(0, 1) + "_" + n.parentNode.parentNode.previousSibling.id.slice(2))),
                  setTimeout(function () {
                      expandBox(n, 0);
                  }, 100),
                  r.focus(),
        ...
        case "ArrowDown":
            t.altKey
                ? ((i = D.getElementById("r_" + n.id.slice(2))),
                  i.nextSibling && ((bM = !0), (n.readOnly = !0), i.parentNode.insertBefore(i, i.nextSibling.nextSibling), (bM = !1), err("fields") ? oErr.refresh() : disableButtons("fields", !1, !1), n.focus()))
                : n.parentNode.parentNode.nextSibling &&
                  (n.type !== "textarea" || n.selectionEnd - n.selectionStart === n.value.length || n.selectionStart === n.value.length) &&
                  (t.preventDefault(),
                  (r = D.getElementById(n.id.slice(0, 1) + "_" + n.parentNode.parentNode.nextSibling.id.slice(2))),
                  setTimeout(function () {
                      expandBox(n, 0);
                  }, 100),
                  r.focus(),
...
function validRow(n, t, i) {
    if (oT.rows.length > 1 && (err("fields") || !t || (oR["r" + n] && t.value !== oR["r" + n][t.id.slice(0, 1)]))) {
        ...
        if (
            w &&
            ((c.childNodes[2].innerHTML =
                f === 0 || f === 4 ? '<textarea id="v_' + n + '" rows="1">' + u.value + "</textarea>" : '<input id="v_' + n + '"' + (f === 1 && oO.mask ? ' type="password"' : "") + ' value="' + escapeQuotes(u.value) + '">'),
            (u = D.getElementById("v_" + n)),
            f === 4 ? ((r.oldValue = r.value), (r.value = ""), (r.disabled = !0)) : (r.value || (r.value = r.oldValue || ""), (r.disabled = !1)),
            l.selectedIndex > 4 ? ((u.oldValue = u.value), (u.value = ""), (u.disabled = !0)) : (u.value || (u.value = u.oldValue || ""), (u.disabled = !1)),
            l.classList.toggle("disabled", f > 1 || h.textContent.indexOf(sBackup) === 0),
            r.disabled ? u.focus() : r.focus(),
            !y && !err("fields"))
        )
            return;
        if (
            (validRegex(r.value)
                ? validRegex(e.value)
                    ? r.value || u.value
                        ? v() && ((r.style.background = u.style.background = e.style.background = ""), (oErr.fields[o] = ""), (s = ""), err("fields") || showStatus())
                        : ((e.style.background = ""), f === 1 || r.disabled ? ((r.style.background = ""), (u.style.background = sErrBg), u.focus()) : ((r.style.background = sErrBg), r.focus()), (oErr.fields[o] = 1))
                    : ((e.style.background = sErrBg), e.focus(), (oErr.fields[o] = 1), (s = chrome.i18n.getMessage("statusInvalidRegex")))
                : ((r.style.background = sErrBg), r.focus(), (oErr.fields[o] = 1), (s = chrome.i18n.getMessage("statusInvalidRegex"))),
            v() || err("fields"))
        ) {
...
function changeCat(n) {
    ...
    if (((isBackupCat = t.indexOf(sBackup) === 0), n instanceof Event)) {
        ...
        if (bFRS === !1) {
            showDiscard();
            oC.selectedIndex = nCI;
            D.getElementById("button-discard").addEventListener(
                "click",
                function () {
        ...
        chrome.storage.local.get("catnow", function () {
            chrome.runtime.lastError || (isBackupCat || BG.updateCatMenu(nCI2), storageSet({ catnow: nCI2 }));
        });
    } else
        (nCI = n),
            isBackupCat || (nCI2 = nCI),
            ...
            chrome.storage.local.get("catnow", function () {
                chrome.runtime.lastError ||
                    (isBackupCat || aC.length + 4 !== oC.length - 1 || BG.updateCatMenu(nCI2),
                    storageSet({ catnow: nCI2 }).then(function () {
                        loadOptions("fields");
                    }));
            });
    loadOptions("cats");
...
function popCats(n) {
    ...
    n > aC.length + 4 && storageSet({ catnow: (n = nCI = nCI2 = 2) });
    oC.selectedIndex = n;
}
function validCat(n, t) {
    ...
    for (u = aC2 || aC, r = 0; r < u.length; ++r) if (u[r].n === n) return alert(chrome.i18n.getMessage("statusProfileDuplicate")), i && t(n), !1;
    return !0;
}
function showBox(n, t, i) {
    ...
    oBC.addEventListener("click", showBox, { once: !0 });
}
function showCats() {
    ...
    var t =
            '<p></p><select id="content-catlist" size="13"></select><button id="button-new" accesskey="n">' +
            chrome.i18n.getMessage("buttonsProfileNew") +
            '</button><br><button id="button-rename" accesskey="r" disabled>' +
            chrome.i18n.getMessage("buttonsProfileRename") +
            '</button><br><button id="button-movefirst" accesskey="i" disabled>' +
            chrome.i18n.getMessage("buttonsProfileMoveFirst") +
            '</button><br><button id="button-moveup" accesskey="u"" disabled>' +
            chrome.i18n.getMessage("buttonsProfileMoveUp") +
            '</button><br><button id="button-movedown" accesskey="w" disabled>' +
            chrome.i18n.getMessage("buttonsProfileMoveDown") +
            '</button><br><button id="button-movelast" accesskey="l" disabled>' +
            chrome.i18n.getMessage("buttonsProfileMoveLast") +
            '</button><br><button id="button-delete" accesskey="t" disabled>' +
            chrome.i18n.getMessage("buttonsProfileDelete") +
            '</button><br><button id="button-duplicate" accesskey="p" disabled>' +
            chrome.i18n.getMessage("buttonsProfileDuplicate") +
            '</button><br><button id="button-sort" accesskey="s"' +
            (aC.length < 2 ? " disabled" : "") +
            ">" +
            chrome.i18n.getMessage("buttonsProfilesSort") +
            "</button><br><br>",
        n = aC.length;
    ...
    nCI > 4 &&
        ((oCL.selectedIndex = nCI - 5),
        oCL.dispatchEvent(eChange),
        (oCL.oldCat = oC[nCI].value),
        setTimeout(function () {
...
function saveCats() {
    ...
    if (oCL.changed) {
        ...
        if (Object.size(oR2) > 0) for (n in oR2) oR[n] = oR2[n];
        nR = Math.max(nR, nR2);
        storageSet({ rulecount: nR, rules: JSON.stringify(oR) }).then(function () {
            saveOptions("cats");
            popCats();
...
function doNewCat(n) {
    var i, r, t;
    (n instanceof Event && (n = ""), (i = (prompt(chrome.i18n.getMessage("promptProfileName") + ":", n || "") || "").trim()), validCat(i, doNewCat)) &&
        ((r = oCL.selectedIndex), (t = { k: "c" + nC2++, n: i, s: "" }), r < 0 ? aC2.push(t) : aC2.splice(r + 1, 0, t), popCatList(!0, t.k), (oCL.newCat = t.k));
}
...
function doCopyCat() {
    var t = oCL.selectedIndex,
        r = "c" + nC2++,
        i = { k: r, n: aC2[t].n + " [Copy]", s: aC2[t].s },
        n;
    t < 0 ? aC2.push(i) : aC2.splice(t + 1, 0, i);
    popCatList(!0, i.k);
    oCL.newCat = i.k;
    for (n in oR) oR[n].c === aC2[t].k && (oR2["r" + nR2++] = { t: oR[n].t, n: oR[n].n, v: oR[n].v, s: oR[n].s, o: oR[n].o, c: r });
}
function doSortCats() {
    aC2.sort(sortByName);
...
function showDiscard() {
    showBox(
        "<p>" + chrome.i18n.getMessage("statusUnsavedChanges") + "</p>",
        chrome.i18n.getMessage("optionsFieldsConfirmation"),
        '<button id="button-discard" data-catnow="' + oC.selectedIndex + '">' + chrome.i18n.getMessage("buttonsDiscardContinue") + "</button>"
    );
}
function showMoveRule(n) {
    for (
        var r = "<p>" + chrome.i18n.getMessage("optionsFieldsSelectProfile") + '</p><p><select id="select-cat" accesskey="p"><option value="">' + chrome.i18n.getMessage("optionsProfilesMenuUnfiled") + "</option>", t = 0;
        t < aC.length;
        ++t
    )
        r += '<option value="' + aC[t].k + '"' + (aC[t].k === n.cat ? " selected" : "") + (aC[t].n.indexOf(sBackup) === 0 ? " hidden" : "") + ">" + aC[t].n + "</option>";
    ...
    var u = n.id.slice(2),
        ...
        f = function (n) {
            ...
            r = "c" + nC++;
            aC.push({ k: r, n: t, s: D.getElementById("s_" + u).value || oCS.value });
            moveRule(u, r, t);
            saveOptions("cats");
        };
...
function moveRule(n, t, i) {
    ...
    nCI > 1
        ...
        : ((u = D.getElementById("m_" + n)), (u.firstChild.textContent = i), (u.title = chrome.i18n.getMessage("optionsProfileTT") + ": " + i + "\n" + chrome.i18n.getMessage("moveRuleTT") + " (" + sK + "+M)"), (r.cat = t));
    storageSet({ rulecount: nR, rules: JSON.stringify(oR) }).then(function () {
        showBox();
...
function importCSV(n) {
    var i = (typeof n == "string" ? n : oIE.value).trim(),
        ...
        c;
    i &&
        (showStatus(chrome.i18n.getMessage("statusPreparingData"), 2),
        setTimeout(function () {
            if (
                ((i = i.replace(/^[ \r\n]*|[ \r\n]*$/g, "")),
                (i = i.replace(/@@/g, "\\@\\@")),
                ...
                / ###,,/.test(i)
                    ...
                    : / ###\t\t/.test(i) && ((i = i.replace(/\t(?=[^"]*"(?:[^"]*"[^"]*")*[^"]*$)/g, "%%")), (i = i.replace(/@@([^"]+?\t)/g, "\n$1")), (u = "\t")),
                (f = i.split("\n")),
                (c = f.length),
                !u || c === 0 || f[0].split(u).length < 5)
            ) {
            ...
            a = setInterval(function () {
                var n;
                for (t = f[h].split(u), n = 0; n < t.length; ++n)
                    (u === "," || /^(".*?""[^"]*"".*?"|"[^"]+")$/.test(t[n])) && ((t[n] = t[n].replace(/^"|"$/g, "")), (t[n] = t[n].replace(/""/g, '"'))),
                        (t[n] = t[n].replace(/%%/g, "\t")),
                        (t[n] = t[n].replace(/~~/g, ",")),
                        (t[n] = t[n].replace(/@@/g, "\n")),
                        (t[n] = t[n].replace(/\\%\\%/g, "%%")),
                ...
                if (((t[0] = t[0].toLowerCase()), /^c\d+/.test(t[0]))) {
                    ...
                    o || (showStatus(chrome.i18n.getMessage("statusImportingProfile", [t[1]]), 2), aC.push({ k: r ? "c" + nC++ : t[0], n: t[1], s: t[2], h: isNaN(parseInt(t[3])) ? t[3] : "" }), (e[t[0]] = aC[aC.length - 1].k));
                    r || (nC = Math.max(nC, +t[0].slice(1)) + 1);
                } else if (/^r\d+/.test(t[0]) || !isNaN(parseInt(t[0])))
                    showStatus(chrome.i18n.getMessage("statusImportingRule", [++v, l]), 2),
                        t.length === 7
                            ? ((oR[r ? "r" + nR++ : t[0]] = { t: +t[1], n: t[2], v: t[3], s: t[4], o: +t[5], c: r ? e[t[6]] : t[6] }), r || (++nR, (s = Math.max(+t[0].slice(1), l, s))))
                            : (oR["r" + nR++] = { t: +t[0], n: t[1], v: t[2], s: t[3], o: +(+t[0] < 2 && oO.overwrite), c: r ? e[t[4]] : t[4] });
                    ...
                    storageSet({ other: oO, backup: +oBU.value, delay: +oD.value, voice: oV.selectedIndex });
                }
                ++h;
...
function exportCSV() {
    ...
    for (n in oR)
        t +=
            ...
            oR[n].s.replace(/"/g, '""') +
            '",' +
            (oR[n].o === undefined ? +(oR[n].t < 2 && oO.overwrite) : oR[n].o) +
            "," +
            oR[n].c +
            "\n";
    t +=
        "### " +
        sExtName.toUpperCase() +
        ' OPTIONS ###,,,,,,\nadvanced,"' +
        JSON.stringify(aA).replace(/"/g, '""') +
        '",,,,,\nexceptions,"' +
        JSON.stringify(aE).replace(/"/g, '""') +
        '",,,,,\ntextclips,"' +
        JSON.stringify(aT).replace(/"/g, '""') +
        '",,,,,\nvariables,"' +
        JSON.stringify(aV).replace(/"/g, '""') +
        '",,,,,';
...
function handleKeyDown(n) {
    D.activeElement.id !== "content-hotkey" &&
        (n.key === "Escape" && oBC && oL.style.display === "flex"
            ? oBC.click()
            : D.body.id === "fields" && n.altKey && n.key === "g"
            ? showCats()
            : D.body.id === "other" && n.altKey && n.key === "p" && oL.style.display !== "flex" && (oIA.checked ? oIA.focus() : oIR.focus()));
}
function init() {
...
var D = document,
    ...
    sBackup = "Backup - ",
    oMenu = { t: ["Text", "Password", "Select", "Checkbox/Radio", "JavaScript"], o: ["Safe", "Overwrite", "Prepend", "Append", "Wrap", "Increment", "Decrement"] },
    oR,
    oR2,
    nR,
    nR2,
    ...
    oErr = { msg: chrome.i18n.getMessage("statusFieldError") },
    err = function (n) {
